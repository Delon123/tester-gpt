...
MODIFIED: Replaced Ollama integration with OpenRouter API.
"""
...
# PENTING: Kunci API OpenRouter Anda yang disuplai oleh pengguna.
OPENROUTER_API_KEY = "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"
# ------------------------------
...
# --- FUNGSI LLM BARU (OPENROUTER) ---

# Fungsi untuk mendapatkan riwayat chat (Hanya diganti namanya dari 'ollama')
def get_chat_history_for_openrouter(chat_id):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
...
# Fungsi untuk streaming respons (Diubah total untuk OpenRouter API)
def stream_openrouter_response(model_name, history, new_message, chat_id):
    messages = history
            ...
            for line in r.iter_lines():
                if line:
                    ...
                    if line_str:
                        ...
                        # Ekstraksi konten dari format OpenRouter/OpenAI
                        ...
                        # Cek apakah respons selesai
                        if data.get("choices", [{}])[0].get("finish_reason"):
                            break

    except Exception as e:
        yield f"[Error: Gagal terhubung ke OpenRouter. Pastikan Kunci API dan Model benar. Detail: {e}]"
    finally:
        if ai_full_response:
            conn = sqlite3.connect(DB_NAME)
            c = conn.cursor()
...
def chat_stream():
    ...
    # PANGGIL FUNGSI OPENROUTER
    history = get_chat_history_for_openrouter(chat_id) 
    
    return Response(stream_with_context(stream_openrouter_response(model_name, history, user_message, chat_id)),
                     mimetype="text/plain")

@drana_infinity.route('/get_models', methods=['GET'])
def get_models():
    # Model OpenRouter yang dipilih (diganti dari Ollama tags)
    models = [
...
if __name__ == '__main__':
    ...
    # Port diatur ke 8000 untuk menghindari masalah hak akses dan port yang sudah digunakan (Errno 98)
    print("Drana-Infinity server is running on ::: http://127.0.0.1:8000")
    serve(drana_infinity, host='127.0.0.1', port=8000)